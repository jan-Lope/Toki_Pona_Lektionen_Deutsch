...
CSV_FILE_DICT="toki-pona_deutsch.csv"
...
expand _build/toki-pona_deutsch.txt | fgrep -v "##" | sed -e 's/  */ /g' | iconv -f ISO-8859-15 -t UTF-8  >> _build/dictionary.coffee
...
cat _build/toki-pona_deutsch.txt | iconv -f ISO-8859-1 -t UTF-8  | sed -e 's#'::'#'@'#g' | fgrep -v "##"  | \
		sed -e 's#'@\ '#'@'#g' | sed -e 's#'@\ '#'@'#g' | sed -e 's#'@\ '#'@'#g' | \
		sed -e 's#'\ @'#'@'#g' | sed -e 's#'\ @'#'@'#g' | sed -e 's#'\"'#'\\\\\"'#g' | \
		fgrep -v "Adjektiv:" | fgrep -v "Substantiv:" | fgrep -v "Ziffer:" | fgrep -v "transitiv:" | fgrep -v " Konjunktion:" | fgrep -v "Interjektion:" | \
		fgrep -v "position:" | fgrep -v "Separator:" | fgrep -v "Subjekt:" | fgrep -v "inoffiziell:" | fgrep -v "transitive:" | fgrep -v "verb:" | \
        awk -F\@ '{print "    {\n    \"entry\" : {\n      \"id\" : "NR+10000 ",\n      \"form\" : \"" $1 "\"\n    },\n    \"translations\" : [ {\n      \"title\" : [ ],\n      \"forms\" : [ \""$2"\" ]\n    } ],\n    \"tags\" : [ ],\n    \"contents\" : [ ],\n    \"variations\" : [ ],\n    \"relations\" : [ ]\n    } , "}'  >> $OTM_JSON_FILE_DICTIONARY_FILE
...
echo "Toki Pona - Woerterbuch $TODAY"                                                                      > $TXT_FILE_WORD_LIST
...
echo "\title{Toki Pona - Wortliste}"                 >> $TEX_FILE_WORD_LIST.tex
...
fgrep "&&" dict.tex | iconv -f ISO-8859-1 -t UTF-8   >> $TEX_FILE_WORD_LIST.tex
...
echo ".TH man 6 $TODAY Toki Pona - Woerterbuch"  > $MAN_FILE_WORD_LIST
echo ".SH NAME"                                 >> $MAN_FILE_WORD_LIST
echo "Toki Pona - Woerterbuch"                  >> $MAN_FILE_WORD_LIST
echo ".SH DESCRIPTION"                          >> $MAN_FILE_WORD_LIST
echo "Das Ziel der von Sonja Lang (2001) kuenstlich geschaffenen Sprache toki pona ist der Minimalismus." >> $MAN_FILE_WORD_LIST
echo "toki pona besteht aus nur etwa 120 Wörtern, die in ihrer Form auch nicht verändert werden." >> $MAN_FILE_WORD_LIST
...
fgrep "&&" dict.tex | iconv -f ISO-8859-1 -t UTF-8 | fgrep "\\" | fgrep -v "%" | sed -e 's#'\dots'#''#g' | sed -e 's#\\#''#g' | \
        ...
        sed -e 's#'\"'#'\'\''#g' | sed -e 's#'\&'#''#g' | \
        sed -e 's#'\ \ '#'\ '#g' | sed -e 's#'\ \ '#'\ '#g' | sed -e 's#'\ \ '#'\ '#g' | sed -e 's#'\ \ '#'\ '#g' | \
        awk -F\@ '{print "    {\n    \"entry\" : {\n      \"id\" : "NR ",\n      \"form\" : \"" $2 "\"\n    },\n    \"translations\" : [ {\n      \"title\" : \"" $4 "\",\n      \"forms\" : [ \""$5"\" ]\n    } ],\n    \"tags\" : [ ],\n    \"contents\" : [ ],\n    \"variations\" : [ ],\n    \"relations\" : [ ]\n    } , "}'  >> $OTM_JSON_FILE_WORD_LIST
sed -i '$ d' $OTM_JSON_FILE_WORD_LIST  # cut last line
cat >> $OTM_JSON_FILE_WORD_LIST <<EOF 
  } ],
  "zpdic" : {
    "alphabetOrder" : "",
    "alphabetOrderType" : "UNICODE",
...
EOF
if [ ! -f $OTM_JSON_FILE_WORD_LIST ]; then
	echo "ERROR"
	exit 1
fi
rm -f _build/$OTM_JSON_FILE_WORD_LIST
cp $OTM_JSON_FILE_WORD_LIST _build/
if [ ! -f _build/$OTM_JSON_FILE_WORD_LIST ]; then
	echo "ERROR"
	exit 1
fi
#
