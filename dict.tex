/%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{toki-pona -- Deutsch W\"orterbuch}
\label{'dict'}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
\textbf{?} && \textit{Separator}: Beendet wird eine Frage (Interrogativ) mit einem Fragezeichen. \\ && Verwende ein Fragezeichen nicht zusammmen mit einem der anderen Separatoren. \\ 
 && \\ % no-dictionary
\textbf{:} && \textit{Separator}: Ein Doppelpunkt trennt ein Hinweis-Satz von einem Satz. \\  && Jeweils vor und nach dem Doppelpunkt muss ein vollständiger Satz sein. \\  && Verwende einen Doppelpunkt nicht zusammmen mit einem der anderen Separatoren. \\ 
 && \\ % no-dictionary
...
\textbf{"} && \textit{Separator}: Anführungszeichen werden für Zitate oder Originalschreibweise verwendet. \\ 
 && \\ % no-dictionary
\textbf{'} && \textit{inoffiziell}: Ein Apostroph kann ein Prädikat, das kein Verb enthält, kennzeichnen.  \\ 
 && \\ % no-dictionary
...
\textbf{ike la \dots} && \textit{Substantiv}: wenn Übel ..., bei Verderbtheit, aus Boshaftigkeit \\ 
\textbf{ike} && \textit{Verb, intransitiv}: schlecht sein, jdm. in den Arsch kriechen \\ 
...
\textbf{jaki!} && \textit{Interjektion}: Igit! Ekelhaft! \\ 
...
\textbf{jaki (e \dots)} && \textit{Verb, transitiv}: verschmutzen, verunreinigen, verpesten \\ 
 && \\ % no-dictionary
\textbf{\dots jan} && \textit{Adjektiv}: persönlich, human, jemandes \\ 
\textbf{\dots jan} && \textit{Adverb}: persönlich, human, jemandes \\ 
...
\textbf{jelo} && \textit{Substantiv}: Gelb, Hellgrün \\ 
 && \\ % no-dictionary
\textbf{\dots jo} && \textit{Adjektiv}: privat, persönlich \\ 
...
\textbf{kasi} && \textit{Substantiv}:  Pflanze, Blatt, Kraut, Baum, Strauch, Gras, Pilz \\ 
...
\textbf{kili} && \textit{Substantiv}: Frucht, Obst, Gemüse, Pilz \\ 
 && \\ % no-dictionary
\textbf{\dots kin} && \textit{Adjektiv}: auch, noch, ebenso, ferner, außerdem, gerade, wirklich, tatsächlich \\ && kin kann das letzte Wort in einer Adjektiv-Gruppe sein. \\
...
%\textbf{kin!} && \textit{Interjektion}: Wirklich! \\ 
 && \\ % no-dictionary
\textbf{kipisi } && \textit{Substantiv} (inoffiziell): Abschnitt, Fragment, Scheibe \\ 
\textbf{kipisi (e \dots)} && \textit{Verb, transitiv} (inoffiziell): schneiden \\ 
 && \\ % no-dictionary
...
\textbf{lili (e \dots)} && \textit{Verb, transitiv}: verkleinern, verringern, ermäßigen, schrumpfen\\ 
 && \\ % no-dictionary
\textbf{\dots linja} && \textit{Adjektiv}: länglich, rechteckig, lang \\ 
...
\textbf{mije} && \textit{Substantiv}: Mann, Kerl, Ehemann, Liebhaber \\ 
 && \\ % no-dictionary
...
\textbf{moku (e \dots)} && \textit{Verb, transitiv}: essen, trinken, schlucken, verzehren, verspeisen \\ 
 && \\ % no-dictionary
\textbf{\dots moli} && \textit{Adjektiv}: tot, tödlich, fatal, schwerwiegend \\ 
...
\textbf{monsi} && \textit{Substantiv}: Rücken, Heck, Hintern, Po, Arsch \\ 
 && \\ % no-dictionary
\textbf{\dots monsuta} && \textit{Adjektiv} (inoffiziell): ängstlich \\ 
\textbf{monsuta} && \textit{Substantiv} (inoffiziell): Monster, Ungeheuer, Fabelwesen, Angst \\ 
 && \\ % no-dictionary
...
\textbf{nasin} && \textit{Substantiv}: Weg, Straße, Strecke, Pfad, Methode, Doktrin, Lehre, System \\ 
 && \\ % no-dictionary
\textbf{\dots nena} && \textit{Adjektiv}: hügelig, bergig, bucklig, holprig \\ 
...
\textbf{nimi (e \dots) } && \textit{Verb, transitiv}: bennenen, Namen geben \\ 
 && \\ % no-dictionary
\textbf{\dots noka} && \textit{Adjektiv}: Fuß-, niedriger, unten \\ 
\textbf{\dots noka } && \textit{adverb}: zu Fuß \\ 
\textbf{noka} && \textit{Substantiv}: Bein, Fuß \\ 
 && \\ % no-dictionary
\textbf{o!} && \textit{Interjektion}: Hallo! (um jemandes Aufmerksamkeit zu erwirken) \\ 
...
\textbf{\dots o \dots !} && \textit{Separator}: Ein \glqq o \grqq wird bei Befehlen verwendet und ersetzt \glqq li \grqq. \\  
 && \\ % no-dictionary
\textbf{\dots oko} && \textit{Adjektiv}: optisch, Augen- \\ 
...
\textbf{pakala} && \textit{Verb, intransitiv}: vermasseln, auseinander fallen, brechen,  \\ 
...
\textbf{\dots pi \dots } && \textit{Separator}: \glqq pi \grqq ermöglicht komplexe Substantive. \\ &&  Es trennt ein Substantiv von einem anderen Substantiv, das mindestens ein Adjektiv hat. \\ && Nach \glqq pi \grqq kann nur ein Substantiv oder Pronomen folgen.  \\  && Verwende ein \glqq pi\grqq nicht zusammmen mit einem der anderen Separatoren. \\ 
 && \\ % no-dictionary
\textbf{\dots pilin} && \textit{Adjektiv}: sensitiv, fühlend, empathisch \\ 
...
% \textbf{\dots poka \dots} && \textit{Präposition}:  in Gesellschaft von, mit, neben \\ 
 && \\ % no-dictionary
\textbf{poki} && \textit{Substantiv}: Container, Behälter, Box, Schüssel, Schale, Tasse, (Trink-) Glas \\ 
...
\textbf{pona la \dots} && \textit{Substantiv}: zum Glück, einfacherweise \\ 
...
\textbf{seli (e \dots)} && \textit{Verb, transitiv}: erhitzen, erwärmen, aufwärmen, kochen \\ 
 && \\ % no-dictionary
...
\textbf{sewi (e \dots)} && \textit{Verb, transitiv}: heben, anheben \\ 
 && \\ % no-dictionary
\textbf{\dots sijelo} && \textit{Adjektiv}:  körperlich, materiell, fleischlich \\ 
...
\textbf{sinpin} && \textit{Substantiv}: Front, Vorderseite, Gesicht, Brust, Brustkorb, Rumpf, Wand, Mauer \\ 
 && \\ % no-dictionary
\textbf{\dots sitelen} && \textit{Adjektiv}: bildlich, figurativ, bildhaft, metaphorisch \\ 
...
\textbf{suwi} && \textit{Substantiv}: Süßigkeiten, Zucker, Schokolade, Bonbon \\ 
...
\textbf{tan} && \textit{Verb, intransitiv}: stammen aus, kommen aus \\ 
 && \\ % no-dictionary
\textbf{\dots taso} && \textit{Adjektiv}: nur, einzig, alleinig \\ 
\textbf{\dots taso} && \textit{Adverb}: nur, einzig, alleinig \\ 
...
\textbf{tawa} && \textit{Verb, intransitiv}: gehen, spazieren, verlassen, abfahren, reisen, besuchen \\ 
...
\textbf{telo (e \dots)} && \textit{Verb, transitiv}: begießen, bewässern, befeuchten, waschen, schmelzen \\ 
 && \\ % no-dictionary
\textbf{\dots tenpo} && \textit{Adjektiv}: zeitlich, chronologisch \\ 
...
\textbf{tenpo} && \textit{Substantiv}: Zeit, Zeitperiode, Zeitabschnitt, Moment, Dauer, Situation \\ 
 && \\ % no-dictionary
\textbf{\dots toki} && \textit{Adjektiv}: eloquent, sprachlich, verbal, grammatisch \\ 
\textbf{\dots toki} && \textit{Adverb}: eloquent, sprachlich, verbal, grammatisch \\ 
