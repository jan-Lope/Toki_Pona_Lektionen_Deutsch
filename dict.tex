...
\textbf{.} && \textit{Separator}: Beendet wird ein Aussagesatz mit einem Punkt. \\ && Verwende einen Punkt nicht zusammmen mit einem der anderen Sepratoren. \\ 
 && \\ % no-dictionary
\textbf{!} && \textit{Separator}: Die Befehlsform und Interjektionen enden mit einem Ausrufungszeichen. \\ && Verwende ein Ausrufungszeichen nicht zusammmen mit einem der anderen Sepratoren. \\ 
 && \\ % no-dictionary
\textbf{?} && \textit{Separator}: Beendet wird eine Frage mit einem Fragezeichen. \\ && Verwende ein Fragezeichen nicht zusammmen mit einem der anderen Sepratoren. \\ 
 && \\ % no-dictionary
\textbf{:} && \textit{Separator}: Ein Doppelpunkt trennt zwei (Halb)-Sätze. \\  && Jeweils vor und nach dem Doppelpunkt muss ein vollständiger Satz sein. \\  && Verwende ein Doppelpunkt nicht zusammmen mit einem der anderen Sepratoren. \\ 
 && \\ % no-dictionary
\textbf{,} && \textit{Separator}: Ein Komma wird nach \glqq o\grqq verwendet wenn man Leutet anspricht. \\ && Optional kann es vor einer Präposition eingefügt werden. \\ && Verwende ein Komma nicht zusammmen mit einem der anderen Sepratoren ausser \glqq li\grqq. \\ 
 && \\ % no-dictionary
...
\textbf{\dots ala} && \textit{Ziffer, Adjektiv}: 0 \\ 
...
\textbf{alasa (e \dots)} && \textit{transitives Verb}: jagen, verfolgen und töten, nach Futter suchen \\ 
 && \\ % no-dictionary
\textbf{\dots ale} && \textit{Adjektiv}: alle(r/s), je, jeweils, komplett, ganz, (ale = ali), (veraltet) \\ 
\textbf{\dots ale} && \textit{Ziffer, Adjektiv}: 100 (offizielles Toki Pona Buch) \\ 
...
\textbf{anpa} && \textit{intransitives Verb}:  sich unterwerfen \\
\textbf{anpa (e \dots)} && \textit{transitives Verb}: niederlassen, absenken, abseilen, herunterholen, besiegen, bezwingen \\ 
 && \\ % no-dictionary
...
\textbf{ante (e \dots)} && \textit{transitives Verb}: wechseln; umschalten, ändern, abändern, verändern, modifizieren \\ 
\textbf{ante la \dots} && \textit{Konjunktiv}: anderenfalls, ansonsten, sonst  \\ 
 && \\ % no-dictionary
...
\textbf{awen} && \textit{Substantiv}: inertia, continuity, continuum, stay \\ 
\textbf{awen} && \textit{intransitives Verb}: bleiben, verbleiben, sich aufhalten, warten \\ 
\textbf{awen (e \dots)} && \textit{transitives Verb}: behalten; nicht weggeben \\ 
 && \\ % no-dictionary
\textbf{\dots e \dots} && \textit{Separator}: Ein \glqq e\grqq leitet das direkte Objekt ein. \\  && Verwende ein \glqq e\grqq nicht zusammmen mit einem der anderen Sepratoren. \\ 
 && \\ % no-dictionary
...
\textbf{esun (e \dots)} && \textit{transitives Verb}: kaufen, verkaufen, tauschen \\ 
 && \\ % no-dictionary
...
\textbf{ijo (e \dots)} && \textit{transitives Verb}: objektivieren \\ 
 && \\ % no-dictionary
...
\textbf{ike} && \textit{intransitives Verb}: schlech sein, jdm. in den Arsch kriechen \\ 
\textbf{ike (e \dots)} && \textit{transitives Verb}: schlecht machen, verschlechtern, schlechten Einfluss auf \\ 
\textbf{ike la \dots} && \textit{Konjunktiv}: leider \\ 
 && \\ % no-dictionary
...
\textbf{jaki (e \dots)} && \textit{transitives Verb}: verschmutzen, verunreinigen, verpesten \\ 
 && \\ % no-dictionary
...
\textbf{jan (e \dots)} && \textit{transitives Verb}: personifizieren, verkörpern, vermenschlichen, personalisieren \\ 
 && \\ % no-dictionary
...
\textbf{jo (e \dots)} && \textit{transitives Verb}: haben, enthalten, umfassen \\ 
kama \textbf{jo (e \dots)} && \textit{transitives Verb}: erhalten, bekommen, kriegen, abkriegen, nehmen \\ 
 && \\ % no-dictionary
...
\textbf{kalama} && \textit{intransitives Verb}: Geräusch machen, laut sein \\ 
\textbf{kalama (e \dots)} && \textit{transitives Verb}: klingen, läuten, klingeln, ein Instrument spielen \\ 
...
\textbf{kama} && \textit{intransitives Verb}: kommen, werden, anfangen, eintreffen, ankommen,  \\ 
\textbf{kama \dots} && \textit{Hilfsverb}: werde(n), er- \\ 
\textbf{kama (e \dots)} && \textit{transitives Verb}: bewerkstelligen, herbeiführen \\ 
 && \\ % no-dictionary
...
\textbf{kasi} && \textit{intransitives Verb}: (selber) wachsen \\ 
\textbf{kasi (e \dots)} && \textit{transitives Verb}: (ein-)pflanzen  \\ 
 && \\ % no-dictionary
\textbf{ken} && \textit{Substantiv}: Möglichkeit, Fähigkeit, Begabung, Erlaubnis, Genehmigung, Freigabe \\ 
\textbf{ken} && \textit{intransitives Verb}: kann, ist fähig zu, ist bemächtigt zu, mögen; dürfen, ist möglich \\ 
\textbf{ken \dots} && \textit{Hilfsverb}: kann ... \\ 
\textbf{ken (e \dots)} && \textit{transitives Verb}:  möglich machen, anordnen, aktivieren, freigeben, erlauben \\ 
\textbf{ken la \dots)} && \textit{Konjunktiv}: es ist möglich, dass, vielleicht \\ 
 && \\ % no-dictionary
\textbf{kepeken} && \textit{Substantiv}: Nutzung, Verwendung, Werkzeug \\ 
\textbf{\dots , kepeken \dots} && \textit{Präposition}: mit, mittels, per \\ 
\textbf{kepeken} && \textit{intransitives Verb}: benutzen, verwenden, anwenden, nutzen, gebrauchen \\ 
...
\textbf{kin la \dots} && \textit{Konjunktiv}: außerdem, zusätzlich ebenfalls \\ 
%\textbf{kin!} && \textit{Interjektion}: Wirklich! \\ 
 && \\ % no-dictionary
\textbf{kipisi (e \dots)} && \textit{transitives Verb}: schneiden \\ 
...
\textbf{kiwen (e \dots)} && \textit{transitives Verb}: verfestigen, verhärten, versteinern \\ 
 && \\ % no-dictionary
\textbf{ko} && \textit{Substantiv}: halbfeste Substanz, Paste, Puder, Gummi, Kleber, Mehl, Brei, Pulver \\ 
\textbf{ko (e \dots)} && \textit{transitives Verb}: zerquetschen, pulverisieren \\ 
 && \\ % no-dictionary
...
\textbf{kon} && \textit{intransitives Verb}: atmen \\ 
\textbf{kon} && \textit{transitives Verb}: wegblasen \\ 
 && \\ % no-dictionary
\textbf{\dots kule} && \textit{Adjektiv}: farbenfreudig \\ 
\textbf{kule} && \textit{Substantiv}: Farbe, Lack \\ 
\textbf{kule (e \dots)} && \textit{transitives Verb}: färben, anstreichen, streichen \\ 
 && \\ % no-dictionary
...
\textbf{kulupu } && \textit{transitives Verb}: montieren, zusammenbauen, zusammenrufen \\ 
 && \\ % no-dictionary
\textbf{\dots kute} && \textit{Adjektiv}: auditiv, feinhörig \\ 
\textbf{kute} && \textit{Substantiv}: Gehör, Hörsinn \\ 
\textbf{kute (e \dots)} && \textit{transitives Verb}: hören, zuhören \\ 
 && \\ % no-dictionary
\textbf{\dots la \dots} && \textit{Separator}: Ein \glqq la\grqq trennt eine Phrase (Satz, Halbsatz oder Konjunktiv) von einem Satz. \\  && Verwende ein \glqq la\grqq nicht zusammmen mit einem der anderen Sepratoren. \\ 
 && \\ % no-dictionary
...
\textbf{lape} && \textit{intransitives Verb}: schlafen, ruhen, dösen, liegen \\ 
\textbf{lape } && \textit{transitives Verb}: bewusstlos schlagen \\ 
 && \\ % no-dictionary
...
\textbf{lawa (e \dots)} && \textit{transitives Verb}: führen, lenken, kontrollieren, steuern, herrschen \\ 
 && \\ % no-dictionary
...
\textbf{len (e \dots)} && \textit{transitives Verb}: (Kleidung) tragen, verkleiden \\ 
 && \\ % no-dictionary
...
\textbf{lete (e \dots)} && \textit{transitives Verb}: erkalten, kühlen, gefrieren, vereisen \\ 
 && \\ % no-dictionary
\textbf{\dots li \dots} && \textit{Separator}: Trennwort zwischen jedem Subjekt außer \glqq mi\grqq und \glqq sina\grqq und seinem Verb. \\ && Verwende ein \glqq li\grqq nicht zusammmen mit einem der anderen Sepratoren. \\  
 && \\ % no-dictionary
...
\textbf{lili (e \dots)} && \textit{transitives Verb}: verkleinern, verringern, ermäßigen, schrumpfen\\ 
 && \\ % no-dictionary
...
\textbf{lon} && \textit{intransitives Verb}: anwesend sein, vorhanden sein, enthalten sein, existieren, leben \\ 
 && \\ % no-dictionary
\textbf{\dots luka} && \textit{Adjektiv}: greifbar, Tast- \\ 
\textbf{\dots luka} && \textit{Ziffer, Adjektiv}: fünf, 5 \\ 
...
\textbf{lukin} && \textit{intransitives Verb}: aussehen, aufpassen (auf), Ausschau halten (nach), achten auf \\ 
\textbf{lukin \dots} && \textit{Hilfsverb}: sehen \\ 
\textbf{lukin (e \dots)} && \textit{transitives Verb}: sehen, betrachten, lesen \\ 
 && \\ % no-dictionary
\textbf{lupa} && \textit{Substantiv}: Loch, Höhle; Bau, Mündung, Öffnung, Ausguss, Fenster, Tür, Tor \\ 
\textbf{lupa (e \dots)} && \textit{transitives Verb}: durchbohren, erstechen, perforieren \\ 
 && \\ % no-dictionary
...
\textbf{mama (e \dots)} && \textit{transitives Verb}: bemuttern \\ 
 && \\ % no-dictionary
...
\textbf{meli} && \textit{Substantiv}: Frau, Dame, Lady, Mädchen, Weibchen, Weib, Ehefrau, Freundin \\ 
 && \\ % no-dictionary
\textbf{mi} && \textit{pronoun}: ich, mich, wir, mein, unser \\ 
 && \\ % no-dictionary
...
\textbf{moku (e \dots)} && \textit{transitives Verb}: essen, trinken, schlucken, verzehren, verspeisen \\ 
 && \\ % no-dictionary
...
\textbf{moli} && \textit{intransitives Verb}: tot sein \\ 
\textbf{moli (e \dots)} && \textit{transitives Verb}: töten \\ 
kama \textbf{moli} && \textit{intransitives Verb}: sterben \\ 
 && \\ % no-dictionary
...
\textbf{mu (e \dots)} && \textit{transitives Verb}: Tiergeräusche machen \\ 
 && \\ % no-dictionary
...
\textbf{musi} && \textit{intransitives Verb}: spielen, Spaß haben \\ 
\textbf{musi (e \dots)} && \textit{transitives Verb}: amüsieren, unterhalten, belustigen, bewirten \\ 
 && \\ % no-dictionary
\textbf{\dots mute} && \textit{Adjektiv}: viel, viele, mehrere, sehr, reichlich, zahlreich, weiter, \\ 
\textbf{\dots mute} && \textit{Ziffer, Adjektiv}: 20 (offizielles Toki Pona Buch) \\ 
...
\textbf{mute (e \dots)} && \textit{transitives Verb}: viel machen, vermehren \\ 
 && \\ % no-dictionary
\textbf{\dots namako} && \textit{Adjektiv}: würzig, pikant \\ 
\textbf{namako} && \textit{Substantiv}: Gewürz, Salz, Nahrungsergänzung, Kräuter \\ 
\textbf{namako (e \dots)} && \textit{transitives Verb}: würzen \\ 
 && \\ % no-dictionary
\textbf{nanpa \dots} && \textit{Ziffer, Adjektiv}: Zum Bilden von Ordnungszahlen. \\ 
\textbf{nanpa} && \textit{Substantiv}: Zahl, Ziffer, Anzahl, Ordnungszahl \\ 
\textbf{nanpa (e \dots)} && \textit{transitives Verb}: zählen, nummerieren, beziffern, rechnen \\ 
 && \\ % no-dictionary
...
\textbf{nasa (e \dots)} && \textit{transitives Verb}: verrückt machen \\ 
 && \\ % no-dictionary
...
\textbf{nena} && \textit{Substantiv}: Beule, Hügel, Berg, Gebirge, Erhebung, Button, Nase, Rundung \\ 
 && \\ % no-dictionary
\textbf{ni} && \textit{pronoun}: diese, dieser, dieses (hier), (das) dort, jenes \\ 
 && \\ % no-dictionary
\textbf{nimi} && \textit{Substantiv}: Wort, Name \\ 
\textbf{nimi (e \dots) } && \textit{transitives Verb}: bennenen, Namen geben \\ 
 && \\ % no-dictionary
...
\textbf{\dots o \dots !} && \textit{Separator}: Ein \glqq o \grqq wird bei Befehlen verwendet und ersetzt \glqq li \grqq. \\  && Verwende ein \glqq o \grqq nicht zusammmen mit einem der anderen Sepratoren \glqq e \grqq, \glqq li \grqq, \glqq pi \grqq. \\ 
 && \\ % no-dictionary
...
\textbf{olin (e \dots)} && \textit{transitives Verb}: (eine Person) lieben \\ 
 && \\ % no-dictionary
\textbf{ona} && \textit{pronoun}: er, sie, es, sie (Plural); seins, ihres, seine, deren \\ 
 && \\ % no-dictionary
\textbf{\dots open} && \textit{Adjektiv}: (er-)öffnend \\ 
\textbf{open} && \textit{Substantiv}: start, beginning, opening \\ 
\textbf{open (e \dots)} && \textit{transitives Verb}: öffnen, andrehen, anmachen, anschalten, beginnen, anfangen \\ 
\textbf{open \dots } && \textit{Hilfsverb}: beginnen, starten \\ 
\textbf{open la \dots} && \textit{Konjunktiv}: Anfangs, zu Beginn  \\ 
 && \\ % no-dictionary\dots
...
\textbf{pakala} && \textit{intransitives Verb}: vermasseln, auseinander fallen, brechend,  \\ 
\textbf{pakala (e \dots)} && \textit{transitives Verb}: verpfuschen, zerstören, ruinieren, verletzen \\ 
 && \\ % no-dictionary
...
\textbf{pali} && \textit{intransitives Verb}: handeln, arbeiten, funktionieren \\ 
\textbf{pali (e \dots)} && \textit{transitives Verb}: tuen, machen, bauen, erstellen \\ 
 && \\ % no-dictionary
\textbf{palisa} && \textit{Substantiv}: Rute, Stab, Stange, Angel, Stock, Stiel, Ast, Stock \\ 
\textbf{palisa (e \dots)} && \textit{transitives Verb}: strecken, sexuell zu erregen \\ 
 && \\ % no-dictionary
\textbf{pan} && \textit{Substantiv}: Getreide, Körner; Gerste, Mais, Hafer, Reis, Weizen, Brot, Nudeln \\ 
\textbf{pan} && \textit{transitives Verb}: sähen, aussähen \\ 
 && \\ % no-dictionary
\textbf{\dots pana} && \textit{Adjektiv}: großzügig \\ 
\textbf{pana} && \textit{Substantiv}: Verabreichung, Transformation, Überführung, Umtausch \\ 
\textbf{pana (e \dots)} && \textit{transitives Verb}: geben, senden, entbinden, strahlen, emittieren \\ 
 && \\ % no-dictionary
\textbf{\dots pi \dots } && \textit{Separator}: \glqq pi \grqq ermöglicht komplexe Substantive. \\ &&  Es trennt ein Substantiv von einem anderen Substantiv, das mindestens ein Adjektiv hat. \\ && Nach \glqq pi \grqq kann nur ein Substantiv oder pronoun folgen.  \\  && Verwende ein \glqq pi\grqq nicht zusammmen mit einem der anderen Sepratoren. \\ 
 && \\ % no-dictionary
...
\textbf{pilin} && \textit{intransitives Verb}: fühlen, spüren, verspüren, empfinden \\ 
\textbf{pilin (e \dots)} && \textit{transitives Verb}: denken, wahrnehmen, berühren, anfassen \\ 
 && \\ % no-dictionary
\textbf{\dots pimeja} && \textit{Adjektiv}: schwarz, dunkel \\ 
\textbf{pimeja} && \textit{Substantiv}: Dunkelheit, Finsternis, Schwärze, Schatten \\ 
\textbf{pimeja (e \dots)} && \textit{transitives Verb}: verdunkeln, schwärzen \\ 
 && \\ % no-dictionary
...
\textbf{pini (e \dots)} && \textit{transitives Verb}: beenden, erledigen, fertigstellen, vollenden  \\ 
...
\textbf{poki } && \textit{transitives Verb}: einpacken, einwecken, einwickeln, einmachen  \\ 
 && \\ % no-dictionary
...
\textbf{pona (e \dots)} && \textit{transitives Verb}: verbessern, fixen, reparieren, festmachen \\ 
\textbf{pona la \dots} && \textit{Konjunktiv}: Glücklicherweise \\ 
 && \\ % no-dictionary
...
\textbf{sama} && \textit{Substantiv}: Gleichheit, Parität, Gleichheit, Identität \\ 
\textbf{\dots , sama \dots} && \textit{Präposition}: wie, dergleichen \\ 
\textbf{sama } && \textit{transitives Verb}: gleichzusetzen, gleich machen, nachäffen \\ 
 && \\ % no-dictionary
...
\textbf{seli (e \dots)} && \textit{transitives Verb}: erhitzen, ermärmen, aufwärmen, kochen \\ 
 && \\ % no-dictionary
\textbf{selo} && \textit{Substantiv}: Außenseite, Oberfläche, Haut, Schale, Rinde, Borke, Gestalt, Form \\ 
\textbf{selo } && \textit{transitives Verb}: schützen, bewachen \\ 
 && \\ % no-dictionary
...
\textbf{sewi} && \textit{intransitives Verb}: aufstehen \\ 
\textbf{sewi (e \dots)} && \textit{transitives Verb}: heben, anheben \\ 
 && \\ % no-dictionary
...
\textbf{sijelo (e \dots)} && \textit{transitives Verb}: heilen \\ 
 && \\ % no-dictionary
...
\textbf{sike (e \dots)} && \textit{transitives Verb}: kreisen, drehen \\ 
 && \\ % no-dictionary
...
\textbf{sin (e \dots)} && \textit{transitives Verb}: erneuern, renovieren, auffrischen, beleben \\ 
 && \\ % no-dictionary
\textbf{sina} && \textit{pronoun}: du, ihr (Plural); deins, euer, euch \\ 
 && \\ % no-dictionary
...
\textbf{sitelen (e \dots)} && \textit{transitives Verb}: zeichnen, malen, schreiben \\ 
 && \\ % no-dictionary
...
\textbf{sona} && \textit{intransitives Verb}: wissen, können, verstehen \\ 
\textbf{sona \dots} && \textit{Hilfsverb}: wissen \\ 
\textbf{sona (e \dots)} && \textit{transitives Verb}: wissen, können, verstehen, zu etwas fähig sein \\ 
kama \textbf{sona (e \dots)} && \textit{transitives Verb}: lernen, studieren \\ 
 && \\ % no-dictionary
...
\textbf{suli (e \dots)} && \textit{transitives Verb}: vergrößern, ausbauen, ausdehnen, verlängern \\ 
 && \\ % no-dictionary
...
\textbf{suno (e \dots)} && \textit{transitives Verb}: scheinen, beleuchten \\ 
 && \\ % no-dictionary
...
\textbf{suwi (e \dots)} && \textit{transitives Verb}: süßen, versüßen \\ 
 && \\ % no-dictionary
...
\textbf{tawa} && \textit{intransitives Verb}: gehen, spazieren, verlassen, abfahren, reisen \\ 
\textbf{tawa (e \dots)} && \textit{transitives Verb}: bewegen, verschieben, verlagern \\ 
 && \\ % no-dictionary
...
\textbf{telo (e \dots)} && \textit{transitives Verb}: begießen, bewässern, befeuchten, waschen, schmelzen \\ 
 && \\ % no-dictionary
...
\textbf{toki} && \textit{intransitives Verb}: reden, sprechen, sich unterhalten, plaudern \\ 
\textbf{toki (e \dots)} && \textit{transitives Verb}: sagen \\ 
 && \\ % no-dictionary
...
\textbf{tomo } && \textit{transitives Verb}: bauen, konstruieren \\ 
 && \\ % no-dictionary
\textbf{\dots tu} && \textit{Ziffer, Adjektiv}: zwei, 2 \\ 
\textbf{tu} && \textit{Substantiv}: Duo, Paar \\ 
\textbf{tu (e \dots)} && \textit{transitives Verb}: verdoppeln, zweiteilen, teilen, zerschneiden \\ 
 && \\ % no-dictionary
...
\textbf{unpa} && \textit{intransitives Verb}: Sex haben \\ 
\textbf{unpa (e \dots)} && \textit{transitives Verb}: mit jemanden schlafen, mit jemanden Sex haben \\ 
 && \\ % no-dictionary
...
\textbf{uta (e \dots)} && \textit{transitives Verb}: küssen, saugen \\ 
 && \\ % no-dictionary
...
\textbf{utala (e \dots)} && \textit{transitives Verb}: schlagen, stoßen, überfallen, kämpfen \\ 
 && \\ % no-dictionary
\textbf{\dots walo} && \textit{Adjektiv}: weiß, hell, bleich \\ 
\textbf{walo} && \textit{Substantiv}: Helligkeit, Weiße \\ 
\textbf{walo (e \dots)} && \textit{transitives Verb}: bleichen \\ 
 && \\ % no-dictionary
\textbf{\dots wan} && \textit{Ziffer, Adjektiv}: 1, eins, ein \\ 
\textbf{wan} && \textit{Substantiv}: Einheit, Element, Partikel, Teil, Abschnitt, Stück \\ 
\textbf{wan (e \dots)} && \textit{transitives Verb}: einigen, vereinigen, zusammenführen \\ 
 && \\ % no-dictionary
...
\textbf{wawa (e \dots)} && \textit{transitives Verb}: bestärken, verstärken, ermächtigen \\ 
 && \\ % no-dictionary
...
\textbf{weka (e \dots)} && \textit{transitives Verb}: wegwerfen, entfernen, abtragen,  \\ 
 && \\ % no-dictionary
...
\textbf{wile (e \dots)} && \textit{transitives Verb}: benötigen, brauchen, müssen, wünschen, wollen, sollen \\ 
\end{supertabular} \\
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% eof
